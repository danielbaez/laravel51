...
@section('content')
	...
	<div class="table-responsive">
	  <table class="table table-striped">
	  	...
	  	<tbody>
	  		@foreach($calls as $c)
	  			<tr><td>{{ $c->id }}</td><td>{{ $c->name }}<br>{{ $c->e }}</td><td>{{ date('d-m-Y h:i A', $c->time) }}</td><td><input class="form-control" type="input" value="{{ $c->phone }}"></td><td><a href="#">Ver</a></td><td><button class="btn btn-success">Llamar</button></td><td>{{ $c->prima }}</td><td>{{ $c->company }}</td><td><a class="btn btn-warning">Actualizar</a></td></tr>
	  		@endforeach
	  	</tbody>
	  	<tbody>
